...
class CrosswordCreator():
    ...
    def select_unassigned_variable(self, assignment):
        ...
        var_domains = {x: y for x, y in self.domains.items() if x in variables}
        lowest_value = min(var_domains.items(), key=lambda x: len(x[1]))
        tied_vars = {x: y for x, y in var_domains.items() if len(y) == len(lowest_value[1])}
        if len(tied_vars) == 1:
